...
public class OpenIDTokenProvider extends AuthorizationCodeAccessTokenProvider {
    ...
    private final com.kris.zuul.sample.security.IdTokenToOAuthAdditionalInfo idTokenToOAuthInfoConverter;

    private static final Logger LOGGER = LoggerFactory.getLogger(OpenIDTokenProvider.class);

    private boolean stateMandatory = true;

    @Autowired
    public OpenIDTokenProvider(com.kris.zuul.sample.security.IdTokenToOAuthAdditionalInfo idTokenToOAuthInfoConverter) {
        super.setStateKeyGenerator(stateKeyGenerator);
        this.idTokenToOAuthInfoConverter = idTokenToOAuthInfoConverter;
    }
    ...
    public OAuth2AccessToken obtainAccessToken(OAuth2ProtectedResourceDetails details, AccessTokenRequest request)
        ...
        Map<String, Object> additionalInfoMap = oauth2AccessToken.getAdditionalInformation();

        com.kris.zuul.sample.security.OAuthAdditionalInfo additionalInfo = idTokenToOAuthInfoConverter.translateToken((String) additionalInfoMap.get("id_token"));
        additionalInfoMap.put("boeing_info", additionalInfo);


        DefaultOAuth2AccessToken writableToken = new DefaultOAuth2AccessToken(oauth2AccessToken);
        writableToken.setAdditionalInformation(additionalInfoMap);
